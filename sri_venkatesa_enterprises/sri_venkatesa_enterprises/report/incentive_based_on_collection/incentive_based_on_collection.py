...
def get_data(filters):
	...
	payment_sales=frappe.db.sql(f'''
		...
		WHERE 
			...
			si.docstatus = 1
		''', as_dict=1)
	sales_invoice=frappe.db.get_list("Sales Invoice", filters=[["docstatus","=", 1],                             
	...
	return [{'category': f"Monthly{'' if eligible or not sales_invoice else ' - Not Eligible'}"}] + (list(sorted(data.values(),key=lambda row: row["category"])))
